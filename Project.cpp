...
float fractionalKnapsack (struct Item items[], int n, int capacity) {
	...
	for(int i=0; i<n; i++) {
		...
		// Taking fractionally
			...
			float percentage= (float)(rem_capacity*100)/items[i].weight;
			tot_profit+=rem_capacity*items[i].ratio;
			cout << "Item ID: " << items[i].id << "\nItem Profit, Percent Taken: " << items[i].ratio*rem_capacity << ", " << percentage << "%";
...
int main () {
	...
	for(int i=0; i<n; i++) {
		if (knapsack[i]==1) {
			...
			cout << "Item Profit, Weight: " <<  p[i] << ", " << w[i] << endl;
			dp_profit+=p[i];
		}
	}
	
	// After doing 0/1 First, you'll get capacity as 0 since you'll be decrementing it... Hence, don't keep it here mah dudeeeee
	// float greedy_profit=fractionalKnapsack(items, n, capacity); 
